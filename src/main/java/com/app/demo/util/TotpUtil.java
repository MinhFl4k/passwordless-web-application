...
public class TotpUtil {
    ...
    public static boolean verifyCode(String secret, String code, int variance)
            throws InvalidKeyException, NoSuchAlgorithmException {
        long timeIndex = System.currentTimeMillis() / 1000 / 30;
        byte[] secretBytes = new Base32().decode(secret);
        for (int i = -variance; i <= variance; i++) {
            long calculatedCode = getCode(secretBytes, timeIndex + i);
            String calculatedCodeStr = String.format("%06d", calculatedCode);
            if (calculatedCodeStr.equals(code)) {
                return true;
            }
        }
        return false;
    ...
    public static String generateQRCode(String otpProtocol) throws Throwable {
        try {
